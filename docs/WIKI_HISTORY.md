...
1. **初回候補取得** — 入力欄のフォーカスまたは「候補を表示」で `GET /api/wiki-history` が呼ばれたとき、キャッシュに履歴がなければシート先頭 N 行（`indexRows`）から三つ組列を一括読取し、インデックスを構築します。起動時・行表示時の自動ウォームアップはAPI集中を避けるため行いません。
...
- 保存先: `...__wiki.json`（構築本体）＋`...__wiki-delta.json`（保存学習差分）
- Redis REST環境変数がある場合は構築済み本体を15分共有し、分散ロックで同時再構築を抑止
