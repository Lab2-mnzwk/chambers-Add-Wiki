...
- **更新タイミング（決まった周期の自動更新は無い）**:
  ...
  - **E: 起動直後の候補ウォームは遅延実行**: bootstrap 直後に候補一覧をまとめて取得すると起動時の API 負荷が集中するため、構造・キュー・初回行の取得が落ち着いた後（`WIKI_WARM_DELAY_MS`＝6秒後）に 1 回だけ `/api/wiki-history` を投げてキャッシュを温める（`wikiWarmedRowsRef` で `indexRows` 変更時のみ再実行）。
