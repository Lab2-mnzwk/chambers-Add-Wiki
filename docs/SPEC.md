...
- **権限不足エラー時の扱い**: `Insufficient Permission` / `The caller does not have permission` / `forbidden` 等は `isPermissionError` で検出し、`403` + 専用文言（`PERMISSION_MESSAGE`：「このGoogleアカウントには作業シートへのアクセス権限がありません…」）に変換。`getBootstrap` はこれを **authRequired（`authMessage` 付き）** として扱い、`LoginPanel` ＋ 専用メッセージを表示する。対象シートが未共有のアカウントでのログイン時などに発生し、別アカウントでの再ログインで解決し得る。
