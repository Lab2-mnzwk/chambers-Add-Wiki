...
- **移動時の探索は途中行を描画しない**: `次の行` / `前の行` は、キュー上の次候補をメモリで特定し、進捗フィルタ有効時は **`fetchRowProbe`（`/api/row?probe=true`＝作業 Status 列の1セルのみ）** でライブ status を確認する。対象外なら描画せず次へ。**着地する行が決まって初めて `fetchRowPayload`（行全体）と `setRowPayload`** するため、スキップ中の途中行はちらつかない（探索中は「次/前の対象行を探索中…」表示）。進捗「すべて」のときはスキップ判定不要のため probe を使わず、着地時の1回取得のみ。該当行が無ければ現在の表示行はそのまま維持（復元不要）。
- **大量スキップ時の負荷軽減**: 1回の移動でライブ status スキップが `NAV_REFRESH_SKIP_THRESHOLD`（=5）以上に達したら、行を1件ずつ確認し続けず **スキップが発生したシートだけ** キュー index を再構築（`/api/queue?refresh=true&sheet=s12,s3`）して最新キューで一気にジャンプする。`sheet` 省略時は全シート refresh（手動「キュー再読込」等）。通常時（スキップ少）は発動しない。
- **移動時のキュー再取得は行わない**: 進捗「すべて」でも `次の行` / `前の行` のたびに `/api/queue` は呼ばない。キュー更新はフィルタ切替・保存応答・手動「キュー再読込」・大量スキップ refresh に限定する（外部編集の反映は「キュー再読込」またはアイドル明けの自動クリア）。
...
- **行を開くたびに自己修復**: `getRow` / `getRowProbe` は読み込んだ行のライブ status をキャッシュ `queueIndex` へ反映する（`withBundle` で1回の read/write に統合）。アプリ外でシートを直接「完了」にした行も、一度開けば以降の判定・保存再計算・再読込で除外される。
