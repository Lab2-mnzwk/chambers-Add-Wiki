...
- **移動時の探索は途中行を描画しない**: `次の行` / `前の行` は、**キャッシュ順の次/前 1 行へ直行**し、ライブ値（Status / Assignee）が現在の絞り込み外なら描画せずスキップして次を取り直す。キャッシュとシートの差異は稀という前提で、判定用の複数行先読みはしない。該当行が無ければ現在の表示行はそのまま維持。
  - **通常経路（probe先行）**: まず `/api/row?probe=true` で **Status / Assignee の2セルだけ**をライブ確認し、対象内なら表示データを取得して着地する。表示データは**行キャッシュを許容**し、キャッシュ値が絞り込みと矛盾するときだけ `fresh=true` で取り直す。フル行読みは**着地行だけ**になり、スキップ1回あたりの読みは2セルで済む。連続スキップが `NAV_REFRESH_SKIP_THRESHOLD`（=5）に達したら該当シートの index を再構築（`/api/queue?refresh=true&sheet=...`）。鮮度が不安なときは手動の「対象行リストを更新」。
  - **変更あり＋裏読みヒット時は楽観着地**: 裏読み済みの次行が絞り込み内なら**先に次行を表示**し、保存（`POST /api/save`）は背景で完了させる。裏読みがまだ進行中でも、クリック直後に「保存しながら…」を表示して保存を開始し、**保存と裏読みを並列**で待つ。従来の「裏読み完了待ち→保存開始」という直列待ちは行わない。裏読み失敗時だけ通常のキャッシュ優先読みへフォールバックし、同じ進行中Promiseと重複して行を取得しない。
    - **直列化は「編集あり」の後続操作だけ**: 背景保存が未完了でも、編集なしの `次の行` / `前の行` / `開く` は待たずに進む（前行の保存と競合しないため）。編集ありの保存・移動・ジャンプは背景保存の完了を待ってから実行する（`pendingSaveRef`）。
    - **保存失敗時**: 楽観着地した行にまだ居れば、元の行と未保存編集へロールバックして通知する（移動先で行った直後の編集は破棄される）。既に別の行へ移動済みなら画面は動かさず「行◯◯の保存に失敗しました」と通知のみ行う（該当行を開き直して再入力）。ロールバック直後に並走中の移動が着地しないよう `rollbackSeqRef` で世代管理する。
    - **並走時のキュー整合**: 背景保存の成功で絞り込み外になった行のローカル除外は関数型 `setQueueRows` で行い、除外済みキーを `backgroundRemovedKeysRef` に記録して、並走中の移動が古いキュースナップショットで着地しても復活させない（キュー再読込時にリセット）。
  - **変更あり＋裏読みなし（`POST /api/save-move`）**: `次の行` / `前の行` は `batchUpdate` と移動先 1 行取得、`開く` は `batchUpdate` と指定行取得を並列実行する。移動先は `getRow(forceFresh=false)` でローカル／共有行キャッシュを優先し、別のVercelインスタンスで裏読み済みの行をSheetsから二重取得しない。同じ行を指定して開く場合だけ、保存後の値を保証するため直列＋`forceFresh=true`。
  ...
  - **次方向だけ条件限定の裏読み**: 行着地直後に、**現在の作業者・進捗フィルタ・列表示条件だけ**でキャッシュ順の次 1 行を `/api/row?bg=true` で裏読みする。行キャッシュを許容し（自分の保存はキャッシュへ反映済み）、レート制限（429）時はリトライで粘らず諦める（`bg=true` → `getRow(background)` → `noRetry`）。条件一致の「次の行」ではその結果を使う。着地値が絞り込み外ならスキップして取り直す。前方向・他条件・キュー再読込時は破棄する。裏読み失敗はブラウザログに `[prefetch] failed ...`、前景フォールバックは `[navigation] next prefetch unavailable ...` として残す。
  - **裏読みの取り直し**: 着地時の1回だけで諦めると、その行に留まる限り裏読みが復活せず、滞在時間に関係なく次の移動が必ず「保存しながら…読み込み中」になる（着地直後は背景保存と裏読みが同時に走り 429 に当たりやすい）。そこで次の 3 つで温め直す。①失敗時に `PREFETCH_RETRY_DELAY_MS`（=3秒）後へ **一度だけ**自動再試行（`nextPrefetchRetryTimerRef`。着地・条件変更・次の warm で破棄）、②**編集が入った時点**（`dirty` が立った時）、③**「次の行」ボタンの hover / focus 時**。②③は `ensureNextPrefetch` が担い、現在行・絞り込み条件・次行が一致する裏読み（完了済み・進行中いずれも）があるときは何もしない。移動処理中（`loading`）は着地時の warm に任せて動かさない。
  - **429/503 リトライの待ち時間上限**: Sheets API がレート制限で `Retry-After`（しばしば10秒超）を返しても、`sheetsFetch` は**最大2秒**（`RETRY_WAIT_CAP_MS`）しか待たずに1回だけ再試行する。従来は Retry-After をそのまま待っていたため、行読みが単発で7〜14秒かかる「フリーズ感」の主因だった。リトライ発生時は `[sheets] 429 ...` を warn ログに残し、発生状況を追跡できるようにする。
  - **訪問済みキャッシュ（前の行の即時化）**: 直近に表示した行の payload をクライアントで最大 10 件 LRU 保持する（`visitedPayloadsRef`）。「前の行」（前の前以前も保持範囲なら同様）や「戻った後の次の行」では、保持分があれば **probe もフル読みもせず即着地**する。保存成功時は保存値でパッチして鮮度を保ち（`patchVisitedPayload` / `applySavedEditsToPayload`）、列表示条件が変わったら無効。編集ありの移動で訪問済みヒット時も、次方向の裏読みヒットと同じ**楽観着地＋背景保存**になる。「対象行リストを更新」「表示中の行を再取得」で破棄する。
  - **保存完了表示は自動で消える**: 「N セルを保存しました。」（同期保存・背景保存いずれも）は `flashMessage` で約1.6秒表示して自動的に消す。処理済みであることが分かりやすく、また画面に残り続けない。表示中に別の状況（次の移動・エラー等）へ切り替わっていた場合は上書きを優先し、古い完了表示は消去しない（`messageSeqRef` で世代管理）。
...
- **表示と取得タイミング**: 候補メニューを開くのは入力欄のフォーカスまたは「候補を表示」操作時のみ（1行内の複数入力欄から `/api/wiki-history` が同時発火するのを防ぐ）。取得後は `タイトル|URL`（タイトルは `/api/link-preview` で取得）または `Wiki該当なし（`-` を入力）` / `Wiki欄変更不要のため入力なし`（空欄正解）と、一致種別・件数を表示。候補が無いときは `候補なし`。クリックでセルへ反映（空欄正解はセルを空に）。
- **裏読み（`warmWikiHistory`）**: 行を開いた `GET /api/row/[sheetRow]` のレスポンス後、サーバー側で候補インデックスをバックグラウンドで温める（`next/server` の `after()`。開いている行のシートを優先して構築し、もう一方のシートは後回しにする）。候補メニューの表示自体は上記どおり操作時のみだが、操作した時点で多くの場合インデックスが温まっているため、共有キャッシュ（Redis/Upstash）や当該インスタンスのローカルキャッシュが切れた直後（コールドスタート）の初回操作時の待ちを短縮できる。
...
- **更新タイミング（決まった周期の自動更新は無い）**:
  ...
  - **自動ウォームアップ**: 上記「裏読み」のとおり、行を開くと候補インデックスをバックグラウンドで温める。画面側が追加でAPIを呼ぶわけではない（候補メニュー表示は操作時のみ）ため、通常作業のAPI負荷はほぼ変わらない。
  - **Vercel（サーバーレス）特有の注意**: `.cache` は `/tmp` に置かれ **実行インスタンス単位**（プロセス間で共有されない）。共有キャッシュ（Redis/Upstash）が設定されていれば、あるインスタンスが構築した候補インデックスは他インスタンスからも再利用され、上記の裏読みとも合わせて「当たったインスタンス次第」の影響は小さくなる。共有キャッシュが未設定・到達不可（`GET /api/cache` の `sharedCache` で確認可）の場合はローカルキャッシュのみとなり、この差が出やすい。この点は `/guide`（使い方ガイド）の「正しいWiki の候補機能」で説明する（作業パネル側には常設の説明は置かない）。
